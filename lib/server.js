...
app._initializeWSS = function(server, callback) {
  var self = this;
  var cb = function(c){
    return function(){
      return c;
    ...
    if (!self._clients[key] || !self._clients[key][id]) {
      self._checkKey(key, ip, function(err) {
        if (!err) {
          ...
          self._configureWS(socket, key, id, token, cb());
        } else {
...
app._configureWS = function(socket, key, id, token, callback) {
  ...
  socket.on('message', function(data) {
      ...
      //do parent callback
      //var result = cb()(socket.id);
      //if (!result){
      //   socket.send(JSON.stringify({ type: 'ERROR', payload: { msg: "Not logged in." } }));
     // }
...
app._initializeHTTP = function(callback) {
  var self = this;
  var cb = function(c){
    return function(){
      return c;
