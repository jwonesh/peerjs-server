...
function ExpressPeerServer(server, options, callback) {
  var app = express();
  var cb = function(c){
    return function(){
      return c;
  ...
  app.on('mount', function() {
    ...
    app._initializeHTTP(cb());
    app._setCleanupIntervals();
    app._initializeWSS(server, cb());
  });

  return app;
}

function PeerServer(options, callback, callback2) {
  var app = express();
  ...
  var peerjs = ExpressPeerServer(server, options, callback2);
  app.use(path, peerjs);
